...
from kb.llm import stream_chat
from kb.vectorstore import query as vs_query


def answer(library_id: int, question: str, history=None) -> tuple:
    """返回 (流式生成器, 检索到的来源片段列表)。"""
    # query 入参为原始问题文本；是否先向量化由所选向量后端决定
    # （Chroma 后端用通义 text-embedding-v3 客户端向量化；腾讯云后端用服务端内置 embedding）
    hits = vs_query(library_id, question, settings.retrieve_top_k)
